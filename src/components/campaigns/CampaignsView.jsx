...
import { fmt, pct, momClass, safeDiv } from '@/lib/vdp/campaignAggregates';
...
function useThemeChartColors() {
  const read = () => {
    if (typeof window === 'undefined') {
      return { accent: '#5FA01F', accentFill: 'rgba(95,160,31,0.13)' };
    }
    const root = document.documentElement;
    const accent = getComputedStyle(root).getPropertyValue('--acc').trim() || '#5FA01F';
    const accentFill = getComputedStyle(root).getPropertyValue('--acc2').trim() || 'rgba(95,160,31,0.13)';
    return { accent, accentFill };
  };
...
function TableLoading({ label, minHeight = 100 }) {
  return (
    <div className="cmp-table-loading" style={{ minHeight, padding: '1.25rem .875rem' }}>
      {label}
    </div>
  );
}

const CAMPAIGN_COMPARE_MODES = [
  { value: 'pop', label: 'PoP' },
  { value: 'mom', label: 'MoM' },
];
...
export default function CampaignsView() {
  ...
  const { from: curFrom, to: curTo, currentPeriodLabel: curLabel } = useOverview();
  ...
  if (!dealersLoading && (!client || isAllDealer || !ga4Id)) {
    return (
      <div className="campaigns-view vdp-view">
  ...
  return (
    <div className="campaigns-view vdp-view">
      ...
      {error && (
        <div
          className="vdp-card cmp-table-error"
      ...
      >
        {isBusy && !daily.length ? (
          <TableLoading label="Loading day-wise views…" minHeight={140} />
        ) : !daily.length ? (
      ...
      <div className="vdp-card" style={{ marginBottom: 16 }}>
        ...
        {isBusy && !sortedCampaigns.length ? (
          <TableLoading label="Loading campaign detail…" minHeight={100} />
      ...
      >
        {isBusy && !dateCampaignMatrix.rows.length ? (
          <TableLoading label="Loading date × campaign matrix…" minHeight={100} />
