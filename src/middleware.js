...
async function enforceDashboardReportAccess(supabase, user, response, request, pathname) {
  // Legacy /dashboard/vdp-lab redirects to Compare in the page; treat as compare access.
  const effectivePath =
    pathname.startsWith('/dashboard/vdp-lab') ? '/dashboard/compare' : pathname;
  const reportKey = reportKeyFromPathname(effectivePath);
