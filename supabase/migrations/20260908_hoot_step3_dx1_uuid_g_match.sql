...
AS $$
  SELECT lower((regexp_match(
    lower(btrim(COALESCE(p_page_path, ''))),
...
BEGIN
  ...
  matched AS (
    ...
    LEFT JOIN LATERAL (
      ...
      WHERE c.customer_name IS NOT NULL
        ...
        AND (
          x.url_path = u.path_key
          OR x.url_path = regexp_replace(u.path_key, 'g$', '')
          OR regexp_replace(x.url_path, 'g$', '') = regexp_replace(u.path_key, 'g$', '')
        )
      LIMIT 1
    ) by_path ON TRUE
    LEFT JOIN LATERAL (
    ...
    LEFT JOIN LATERAL (
      ...
      WHERE by_path.sk IS NULL
        ...
        AND c.customer_name IS NOT NULL
        AND x.customer_name_key = LOWER(TRIM(c.customer_name))
        AND u.dx1_id IS NOT NULL
        AND x.dx1_id IS NOT NULL
  ...
  SELECT
    ...
    -- Keep Final VDP aligned with GA4 VDP rows even when inventory URL is missing
    CASE WHEN m.page_path IS NOT NULL AND m.page_path <> '' THEN TRUE ELSE FALSE END AS vdp_conditions,
...
GRANT EXECUTE ON FUNCTION public.build_smart_final_data(text, integer, date, date)
  TO service_role;
GRANT EXECUTE ON FUNCTION public.extract_dx1_listing_id_from_page_path(text)
  TO service_role;
