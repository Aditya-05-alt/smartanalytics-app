-- Split Step 3:
--   build_smart_final_data      = ORIGINAL fast path (page_path only) for all normal dealers
--   build_smart_final_data_qs   = page_path_q_s / ga4_effective_page_path for opted-in dealers
--                                (Destination Cycle 1421445735 only)

-- ---------------------------------------------------------------------------
-- 1) Restore original fast build_smart_final_data
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2) New QS / Destination Cycle path (separate function)
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.build_smart_final_data_qs(
  p_client_id text DEFAULT NULL,
  p_days_back integer DEFAULT NULL,
  p_date_from date DEFAULT NULL,
  p_date_to date DEFAULT NULL
...
BEGIN
  ...
  WITH ga4_tagged AS (
    SELECT
      g.*,
      public.ga4_effective_page_path(g.page_path, g.page_path_q_s) AS match_path
    FROM public.smart_ga4_page_data g
    WHERE g.vdp_conditions = TRUE
      AND (p_client_id IS NULL OR g.client_id = p_client_id)
      AND (p_date_from IS NULL OR g.report_date >= p_date_from)
  ...
  ga4_unique AS (
    SELECT
      ...
      COALESCE(SUM(g.new_users), 0)::INT   AS new_users
    FROM ga4_tagged g
    WHERE g.match_path IS NOT NULL
      AND btrim(g.match_path) <> ''
    GROUP BY g.client_id, g.report_date, g.match_path
  ),
  config_unique AS (
    SELECT DISTINCT ON (
      h.ga4_customer_id,
...
GRANT EXECUTE ON FUNCTION public.build_smart_final_data_qs(text, integer, date, date)
  TO service_role;

COMMENT ON FUNCTION public.build_smart_final_data(text, integer, date, date) IS
  'Step 3 fast path: groups/matches on page_path only (all normal dealers).';

COMMENT ON FUNCTION public.build_smart_final_data_qs(text, integer, date, date) IS
  'Step 3 QS path: groups/matches via ga4_effective_page_path (Destination Cycle / page_path_q_s dealers).';
