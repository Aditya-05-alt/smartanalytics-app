-- Speed up Step 3: when p_client_id is set, only load that dealer's hoot config + inventory
-- (was scanning all ~76k inventory rows on every 1-day rebuild).
...
COMMENT ON FUNCTION public.build_smart_final_data(text, integer, date, date) IS
  'Step 3 fast path: page_path match; scopes inventory to p_client_id when set.';

COMMENT ON FUNCTION public.build_smart_final_data_qs(text, integer, date, date) IS
  'Step 3 QS path: ga4_effective_page_path; scopes inventory to p_client_id when set.';
