...
AS $$
  ...
  agg AS (
    ...
    FROM base
    -- Unmatched inventory (blank inv_make) must not surface as "Unknown" in charts,
    -- especially under channel filter (GA4 VDP join â†’ final without inv_*).
    WHERE make_bucket IS DISTINCT FROM 'Unknown'
      AND NULLIF(TRIM(make_bucket), '') IS NOT NULL
...
AS $$
  ...
  agg AS (
    ...
    FROM base
    WHERE model_bucket IS DISTINCT FROM 'Unknown'
      AND NULLIF(TRIM(model_bucket), '') IS NOT NULL
...
AS $$
  ...
  agg AS (
    ...
    FROM base
    WHERE year_bucket IS DISTINCT FROM 'Unknown'
      AND NULLIF(TRIM(year_bucket), '') IS NOT NULL
...
AS $$
  ...
  agg AS (
    ...
    FROM base
    WHERE type_bucket IS DISTINCT FROM 'Unknown'
      AND NULLIF(TRIM(type_bucket), '') IS NOT NULL
...
AS $$
  ...
  agg AS (
    ...
    FROM base
    WHERE condition_bucket IS DISTINCT FROM 'Unknown'
      AND NULLIF(TRIM(condition_bucket), '') IS NOT NULL
...
-- If dealer has exactly ONE row in smart_dealer_locations, rename Unknown â†’ that name.
-- Forwards p_channels so channel filter works the same as other inventory breakdowns.
...
BEGIN
  ...
  -- 0 or 2+ configured names â†’ base breakdown; drop Unknown bucket.
  IF COALESCE(v_loc_count, 0) <> 1 THEN
    RETURN QUERY
    WITH base AS (
      SELECT lb.location_bucket, lb.views
      ...
      WHERE lb.location_bucket IS DISTINCT FROM 'Unknown'
        AND NULLIF(TRIM(lb.location_bucket), '') IS NOT NULL
    ),
    ranked AS (
      SELECT
        b.location_bucket,
        b.views,
        ROW_NUMBER() OVER (ORDER BY b.views DESC, b.location_bucket)::int AS rank
      FROM base b
      WHERE b.views > 0
    ),
    grand AS (
  ...
  -- Exactly one hardcoded name: remap Unknown â†’ that name.
  RETURN QUERY
  WITH base AS (
    SELECT
      CASE
