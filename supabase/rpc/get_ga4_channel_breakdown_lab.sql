-- LAB ONLY — same filtered logic as live get_ga4_channel_breakdown.
-- Fast path + path equality join + vdp_location_filter_match.
...
BEGIN
  ...
  -- Filtered path: path equality join only (no LIKE on page_location — that timed out).
  RETURN QUERY
  WITH pages AS (
    SELECT
      p.channel,
  ...
  combined AS (
    ...
    -- VDP: match filtered smart_final_data paths by exact path
    SELECT p.channel, p.views
    FROM pages p
...
COMMENT ON FUNCTION public.get_ga4_channel_breakdown_lab(
  ...
  'VDP Lab channel breakdown — fast path = live; filtered = path join + location match. Chunk on API.';
