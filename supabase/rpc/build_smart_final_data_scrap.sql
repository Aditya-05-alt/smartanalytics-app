...
BEGIN
  ...
  ga4_unique AS (
    SELECT
      ...
      g.report_date,
      -- Prefer pathname+query (page_path_q_s) so Dealer Spike default.asp?id=… stay distinct
      public.ga4_effective_page_path(g.page_path, g.page_path_q_s) AS page_path,
      lower(btrim(public.ga4_effective_page_path(g.page_path, g.page_path_q_s))) AS path_key,
      public.extract_vin_from_text(
  ...
  SELECT
    ...
    m.last_seen,
    CASE
      -- ga4_unique is already VDP-tagged; keep Final VDP even if inventory URL missing
      WHEN m.page_path IS NOT NULL AND m.page_path <> '' THEN TRUE
      ELSE FALSE
