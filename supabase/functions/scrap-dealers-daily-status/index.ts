/**
 * Daily scrap-dealers transfer email via Google SMTP (nodemailer).
 *
 * Lists every scrap_link = on dealer (all names) and whether today's scrap
 * data was transmitted, with unit counts.
 *
 * Scrap source (same as inventory-daily-email):
...
const SMTP_PASS = "hrrttkyiuxbexiue";
...
const PAGE = 1000;
const EMAIL_KIND = "scrap_daily_status";
...
type StatusRow = {
  ...
  daily_snapshot_units: number;
  day_complete_units: number;
...
function countScrapByDealer(scrapRows: Record<string, unknown>[]) {
  const byId = new Map<string, number>();
  const byName = new Map<string, number>();
  for (const row of scrapRows) {
    const id = keyId(row.customer_id);
...
function buildStatusRows(
  dealers: ScrapDealer[],
  scrapRows: Record<string, unknown>[],
  reportDate: string,
): StatusRow[] {
  const { byId, byName } = countScrapByDealer(scrapRows);
  return dealers
    .map((d) => {
      const id = keyId(d.ga4_customer_id);
      const name = String(d.customer_name ?? "").trim();
      const units = byId.get(id) || byName.get(keyName(name)) || 0;
      return {
        ...
        daily_snapshot_units: units,
        day_complete_units: units,
        last_seen: null,
        completed_at: null,
      };
    })
...
function formatReport(
  ...
  const lines = [
    ...
    `Scrap as of : ${scrapAsOf}`,
    `Source      : ${scrapSource}`,
    `Dealers     : ${total}  |  Transmitted: ${yes}  |  Missing: ${no}`,
    `Units today : ${unitsToday.toLocaleString("en-IN")}`,
    "============================================================",
    `${pad("#", 3)}  ${pad("Status", 6)}  ${pad("Dealer", 36)}  ${pad("Units", 8, "right")}`,
    "------------------------------------------------------------",
  ];
  ...
  lines.push("------------------------------------------------------------");
  if (missing.length) {
    lines.push("MISSING (no scrap data today):");
    for (const row of missing) {
      lines.push(`  - ${row.dealer_name}  (${row.ga4_customer_id})`);
    }
  } else {
    lines.push("All scrap dealers transmitted data today.");
  }
  lines.push("============================================================");
  return lines.join("\n");
}
...
function buildHtmlEmail(opts: {
  ...
  const { reportDate, rows, scrapAsOf, scrapSource, filename } = opts;
  ...
  const missingHtml = no.length
    ? no
        ...
        .join("")
    : `<li style="font-size:13px;color:#15803d;">All scrap dealers transmitted data today.</li>`;
...
<head>
  ...
  <title>Scrap Dealers Daily Status</title>
</head>
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:640px;background:#ffffff;border:1px solid #e2e8f0;border-radius:12px;overflow:hidden;">
          <tr>
            <td style="padding:28px 28px 20px;background:#0f172a;">
              ...
              <h1 style="margin:0;font-size:24px;line-height:1.25;font-weight:700;color:#ffffff;">Scrap Dealers Daily Status</h1>
              <p style="margin:10px 0 0;font-size:14px;line-height:1.5;color:#cbd5e1;">All scrap dealers · ${escapeHtml(displayDate)}</p>
            </td>
          </tr>
          <tr>
            <td style="padding:24px 28px;">
              <p style="margin:0 0 18px;font-size:14px;line-height:1.55;color:#334155;">
                Daily check for <strong style="color:#0f172a;">every scrap dealer</strong>.
                Scrap from <strong>today’s daily snapshot</strong> (pull_date ${escapeHtml(scrapAsOf)})
                — not the full historical scrap table.
              </p>
                ...
                <tr>
                  <td width="50%" style="padding:0 6px 12px 0;vertical-align:top;">
                    <div style="border:1px solid #e2e8f0;border-radius:10px;padding:14px 16px;background:#f8fafc;">
                      <p style="margin:0 0 4px;font-size:12px;color:#64748b;">Scrap dealers</p>
                      <p style="margin:0;font-size:22px;font-weight:700;color:#0f172a;">${rows.length.toLocaleString("en-IN")}</p>
                    </div>
                  </td>
                  <td width="50%" style="padding:0 0 12px 6px;vertical-align:top;">
                    <div style="border:1px solid #e2e8f0;border-radius:10px;padding:14px 16px;background:#f8fafc;">
                      <p style="margin:0 0 4px;font-size:12px;color:#64748b;">Units today</p>
                ...
                <tr>
                  <td width="50%" style="padding:0 6px 0 0;vertical-align:top;">
                    <div style="border:1px solid #e2e8f0;border-radius:10px;padding:14px 16px;background:#f0fdf4;">
                      ...
                      <p style="margin:0;font-size:20px;font-weight:700;color:#15803d;">${yes.length.toLocaleString("en-IN")}</p>
                    </div>
                  </td>
              ...
              <h2 style="margin:0 0 10px;font-size:15px;font-weight:600;color:#0f172a;">All scrap dealers</h2>
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border:1px solid #e2e8f0;border-radius:10px;overflow:hidden;margin:0 0 22px;">
                ...
                ${dealerHtml || `<tr><td colspan="3" style="padding:12px;font-size:13px;color:#64748b;">No scrap dealers (scrap_link = on)</td></tr>`}
              </table>
              ...
              <p style="margin:0;font-size:12px;color:#94a3b8;">Source: ${escapeHtml(scrapSource)} · CSV attached: ${escapeHtml(filename)}</p>
            </td>
          </tr>
          <tr>
            <td style="padding:16px 28px 24px;border-top:1px solid #e2e8f0;background:#f8fafc;">
              <p style="margin:0;font-size:12px;line-height:1.5;color:#94a3b8;">
                Automated daily email from Scrap Dealers Status · Smart Analytics
              </p>
            </td>
          </tr>
        </table>
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (skipIfSentToday) {
      ...
      if (prior?.some((row) => row?.meta?.kind === EMAIL_KIND)) {
        return jsonRes({
          ...
          ok: true,
          skipped: true,
          message: "Already sent scrap daily status successfully today",
          reportDate,
        });
      }
    }
    ...
    console.log(`[scrap-dealers-daily-status] Loading scrap dealers + today's scrap for ${reportDate}…`);
    ...
    if (dealerErr) {
      throw new Error(
        `${dealerErr.message} — deploy supabase/rpc/get_scrap_dealers_for_sync.sql`,
      );
    }

    const dealers = ((dealerRows || []) as ScrapDealer[]).filter(
      (d) => keyId(d.ga4_customer_id) || String(d.customer_name ?? "").trim(),
    );

    const scrapPack = await fetchTodayScrap(supabase, reportDate);
    const statusRows = buildStatusRows(dealers, scrapPack.rows, reportDate);
    ...
    const template = formatReport(
      statusRows,
      reportDate,
      scrapPack.scrapSource,
      scrapPack.scrapAsOf,
    );

    console.log(
      `[scrap-dealers-daily-status] dealers=${statusRows.length} scrapRows=${scrapPack.rows.length} via ${scrapPack.scrapSource}`,
    );
    console.log(`\n${template}\n`);

    await supabase.from("smart_scrap_daily_status").upsert(
      statusRows.map((r) => ({
        ...
        daily_snapshot_units: r.daily_snapshot_units,
        day_complete_units: r.day_complete_units,
    ...
    const filename = `scrap-dealers-daily-status-${reportDate}.csv`;
    ...
    const fromName = cfg?.from_name || "Scrap Dealers Status";
    const from =
    ...
    const subjectPrefix = cfg?.subject_prefix || "Scrap Dealers Daily Status";
      ...
      if (logId) {
        await supabase
          ...
          .update({
            ...
            ok: true,
            recipients,
            scrap_rows: scrapPack.rows.length,
    ...
    console.log("[scrap-dealers-daily-status] sending SMTP …", {
      reportDate,
      to: recipients,
      cc,
      from,
    ...
    console.log("[scrap-dealers-daily-status] EMAIL SENT OK", {
    ...
    console.error("[scrap-dealers-daily-status] EMAIL NOT SENT —", message);
    if (logId) {
      await supabase
        .from("smart_inventory_email_log")
        .update({
